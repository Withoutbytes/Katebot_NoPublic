...
{
    uintptr_t v;
    v = mem->FindExport(mem->GetModulePtr(module_name), xorstr("CreateInterface").crypt_get());
...
void cvar::Init(void)
{
    auto t = interfaces::find(xorstr("vstdlib.dll").crypt_get());
    cs_vt_cvar = t.find(xorstr("VEngineCvar").crypt_get());
}
