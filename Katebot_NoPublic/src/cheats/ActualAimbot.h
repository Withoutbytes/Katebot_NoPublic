...
public:
	...
	float GetSensitivity(void) {
		static cs_convar sensitivity = cvar::find(xorstr("sensitivity").crypt_get());

		return sensitivity.GetFloat();
	}

	float GetMouseYaw(void) {
		static cs_convar m_yaw = cvar::find(xorstr("m_yaw").crypt_get());

		return m_yaw.GetFloat();
	}

	float GetMousePitch(void) {
		static cs_convar m_pitch = cvar::find(xorstr("m_pitch").crypt_get());
