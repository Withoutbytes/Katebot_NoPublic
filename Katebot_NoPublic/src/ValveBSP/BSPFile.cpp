...
bool BSPFile::parse( const std::string& bsp_directory, const std::string& bsp_file )
{
    ...
    try {
        ...
        if( m_BSPHeader.m_Version < BSPVERSION ) {
            std::cout << xorstr("BSPFile::parse(): ").crypt_get() << bsp_file << xorstr("has an unknown BSP version, trying to parse it anyway...").crypt_get() << std::endl;
        }
        if( m_BSPHeader.m_Ident != IDBSPHEADER ) {
            std::cout << "BSPFile::parse(): " << bsp_file << xorstr("isn't a (valid) BSP file!").crypt_get() << std::endl;
            return false;
        }
        ...
        print_exception( xorstr("parse").crypt_get(), e );
        return false;
    }
...
bool BSPFile::parse_planes( std::ifstream& bsp_binary )
{
    try {
        ...
        print_exception( xorstr("parse_planes").crypt_get(), e );
        return false;
    }
    return true;
}
...
bool BSPFile::parse_nodes( std::ifstream& bsp_binary )
{
    try {
        ...
        print_exception( xorstr("parse_nodes").crypt_get(), e );
        return false;
    }
    return true;
}
...
bool BSPFile::parse_leaffaces( std::ifstream& bsp_binary )
{
    try {
        ...
        if( num_leaffaces > MAX_MAP_LEAFBRUSHES ) {
            std::cout << xorstr("BSPFile::parse_leaffaces(): map has to many leaffaces, parsed more than required..").crypt_get() << std::endl;
        }
        else if( !num_leaffaces ) {
            std::cout << xorstr("BSPFile::parse_leaffaces(): map has no leaffaces to parse!").crypt_get() << std::endl;
        }
    }
    catch( const std::exception& e ) {
        print_exception( xorstr("parse_leaffaces").crypt_get(), e );
        return false;
    }
    return true;
}
...
bool BSPFile::parse_leafbrushes( std::ifstream& bsp_binary )
{
    try {
        ...
        const auto num_leaffaces = m_Leaffaces.size();
        if( num_leaffaces > MAX_MAP_LEAFBRUSHES ) {
            std::cout << xorstr("BSPFile::parse_leaffaces(): map has to many leafbrushes, parsed more than required..").crypt_get() << std::endl;
        }
        else if( !num_leaffaces ) {
            std::cout << xorstr("BSPFile::parse_leaffaces(): map has no leafbrushes to parse!").crypt_get() << std::endl;
        }
    }
    catch( const std::exception& e ) {
        print_exception( xorstr("parse_leafbrushes").crypt_get(), e );
        return false;
    }
    return true;
}
...
bool BSPFile::parse_polygons( void )
{
    try {
        ...
        print_exception( xorstr("parse_polygons").crypt_get(), e );
        return false;
    }
    return true;
}
...
{
    std::cout << xorstr("BSPFile::").crypt_get()
        << ( function_name.empty() ? xorstr("print_exception").crypt_get() : function_name ).c_str()
        << xorstr("() exception(").crypt_get()
        << e.what()
        << xorstr(") while parsing map: ").crypt_get()
        << m_FileName
        << std::endl;
}